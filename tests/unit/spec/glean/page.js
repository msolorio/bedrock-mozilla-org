...
describe('page.js', function () {
    ...
    it('should register a page view correctly', async function () {
        ...
        const ping = pageViewPing.testBeforeNextSubmit(async function () {
            const path = await pageMetrics.path.testGetValue();
    ...
    it('should record specific query parameters in the page view', async function () {
        ...
        let validatorRun = false;
        const ping = pageViewPing.testBeforeNextSubmit(async function () {
            const source = await pageMetrics.queryParams[
                'utm_source'
            ].testGetValue();
            expect(source).toEqual('test-source');

            const campaign = await pageMetrics.queryParams[
                'utm_campaign'
            ].testGetValue();
            expect(campaign).toEqual('test-campaign');

            const medium = await pageMetrics.queryParams[
                'utm_medium'
            ].testGetValue();
            expect(medium).toEqual('test-medium');

            const content = await pageMetrics.queryParams[
                'utm_content'
            ].testGetValue();
            expect(content).toEqual('test-content');

            const entrypointExperiment = await pageMetrics.queryParams[
                'entrypoint_experiment'
            ].testGetValue();
            expect(entrypointExperiment).toEqual('test_entrypoint_experiment');

            const entrypointVariation = await pageMetrics.queryParams[
                'entrypoint_variation'
            ].testGetValue();
            expect(entrypointVariation).toEqual('1');

            const experiment = await pageMetrics.queryParams[
                'experiment'
            ].testGetValue();
            expect(experiment).toEqual('test-experiment');

            const variation = await pageMetrics.queryParams[
                'variation'
            ].testGetValue();
            expect(variation).toEqual('1');

            const v = await pageMetrics.queryParams['v'].testGetValue();
            expect(v).toEqual('1');

            const xv = await pageMetrics.queryParams['xv'].testGetValue();
    ...
    it('should not record unspecified query params in the page view', async function () {
        ...
        let validatorRun = false;
        const ping = pageViewPing.testBeforeNextSubmit(async function () {
            const unspecifiedParam = await pageMetrics.queryParams[
                'unspecified_param'
            ].testGetValue();
